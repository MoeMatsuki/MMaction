...
dir = "/home/moe/MMaction/data/20230703_サンプル"
img_dir = "/home/moe/MMaction/data/20230703_サンプル/convert_img"
...
def process(out_video, dir, vid_name):
    rf_res_path = os.path.join(dir, os.path.join(vid_name, "rf_result.csv"))
    mm_res_path = os.path.join(dir, os.path.join(vid_name, "test_mmaction.csv"))
...
def main():
    ...
    vid_name = "Act01_230703_sample_2023-07-03.zip"
    out_video = os.path.join(dir, "rf_result.mp4")
    process(out_video, dir, vid_name)

    ## Case2
    # for curDir, _, files in os.walk(dir):
    #     if "rf_result.csv" in files:            
    #         vid_name = curDir.split("/")[-1]
    #         dir = "/".join(curDir.split("/")[:-1])
    #         print(vid_name)
