        ...
        for j,file in enumerate(files):
            ...
            for i,data in enumerate(f):
                ...
                if int(tmp[0]) == 0:
                    ...
                    max_x = tmp[1] + (tmp[3] / 2)
                    max_y = tmp[2] + (tmp[4] / 2)
                    result = result +f"{video_id},{str(j).zfill(4)},{round(min_x, 3)},{round(min_y, 3)},{round(max_x, 3)},{round(max_y, 3)},{int(tmp[0])},{person_id}\n"
            f.close()
        return result
    ...
    def save_label_csv(self, dirs, out_csv):
        ...
        for curDir, _, _ in os.walk(dirs):
            ...
            if len(annotation_json_files) > 5:
                list_res = self.json2csv(annotation_json_files, video_name)
...
def main():
    ...
    # img_dir = "/home/moe/MMaction/fastlabel1/train_img/IMG_0567"
    # save_dir = "/home/moe/MMaction/fastlabel1/convert_img/IMG_0567"
