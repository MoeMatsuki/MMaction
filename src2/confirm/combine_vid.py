...
for curDir, dirs, files in os.walk(dir):
    if "rf_result.mp4" in files:
        path_out = curDir + ".mp4"
        high = os.path.join(curDir, "rf_result.mp4")
        low = os.path.join(curDir, "test_mmaction.mp4")

        # ここからメイン実行文
        movie1 = [high, True]     # 元動画のパス1, カラーはTrue
        movie2 = [low, True]    # 元動画のパス2, 白黒はFalse
        scale_factor = 1 
