...
def parse_args():
    ...
    parser.add_argument('--video', default='/home/moe/MMaction/tmp/IMG_1802')
...
def main():
    ...
    if args.outdir is None:
        ...
        process(args.video, args.out, config)
    else:
        # df = pd.read_csv(config.anno_train_csv, header=None)
        df = pd.read_csv(config.anno_val_csv, header=None)
        videos = df.loc[:, 0].unique()
        print("video_num: " + str(len(videos)))
        for video in videos:
            print(video)
            out = os.path.join(args.outdir, video)
            os.makedirs(out, exist_ok=True)
            process(os.path.join(config.data_root, video), out, config)
