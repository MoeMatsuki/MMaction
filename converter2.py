...
def convert_spatio_label():
    ...
    files.sort()
    print(os.path.join(OUT_PATH, "train.csv"))
    # train.txtにファイル書き込み
    with open(os.path.join(OUT_PATH, "train.csv"),"a") as ff:
        for j,file in enumerate(files):
            ...
            for i,data in enumerate(f):
                ...
                # else:
                    # person_id  += 1
                    # ファイル書き込み (video_id,frame,yolo_coorsinate,actionID,personID)
                    # ff.write(f"{video_id},{j},{tmp[1]},{tmp[2]},{tmp[3]},{tmp[4]},{int(tmp[0])},{person_id}\n")
                    # a = 0
