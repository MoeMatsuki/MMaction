...
class RenderingRF:
    def __init__(self):
        self.output_fps = 4
        self.box_col = ["min_x","min_y","max_x","max_y"]
        self.label_map = {1: "teamBD(wb_mtg)", 2: "solo", 3:"teamTH", 4: "teamBD(F2F)"}

    def combin_result(self, mm_df, rf_df):
        rf_bboxes = np.array([self.calc_center(box) for box in rf_df.loc[:, self.box_col].values])
        high_activities = rf_df.loc[:, "high_activity"].values
        
        mm_bboxes = np.array([self.calc_center(box) for box in mm_df.loc[:, self.box_col].values])
        # mmactionの回答１つづつに最も近いrfの結果を探す
        close_result = self.search_pair(mm_bboxes, rf_bboxes)
        h_activities = []
        ...
        mmboxes = mm_df.loc[:, self.box_col].values
    ...
    def search_pair(self, mm_list, rf_list):
        results = {}
...
if __name__ == '__main__':
    ...
    input_dir = "/home/moe/MMaction/data/2022年8月｜6F改修前/mm_result"
    img_dir = "/home/moe/MMaction/data/2022年8月｜6F改修前/convert_img"

    rendering = RenderingRF()
    rendering.main(input_dir, img_dir)
